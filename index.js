...
app.post("/register", requireLoggedOutUser, (req, res) =>
{
    ...
    hash(password).then(hash =>
    {
        ...
        addUser(first, last, email, hash).then(results =>
        {
            ...
            if (matchValue === true) {
                ...
                checkIfSig(userId).then(results =>
                {
                    ...
                    if (req.session.sigId) {
                        //console.log("there is a signature")
...
app.post("/petition", requireNoSignature, (req, res) =>
{
    ...
    if (sig == "") {
        ...
        addSignature(sig, userId).then(results =>
        {
            req.session.sigId = results.rows[0].id // id in the signature table
...
app.get("/thanks", requireSignature, (req, res) =>
{
    ...
    getSignature(id).then(results =>
    {
        ...
        let signatureData = results.rows[0]; // here is name, last and sig
...
app.get("/signers/:city", requireSignature, (req, res) =>
{
    ...
    getSignersByCity(city).then(results =>
    {
        console.log("/profile GET route signers:city results: ", results)
        let signers = results.rows
...
app.post("/profile/edit", requireLoggedInUser, (req, res) =>
{
    ...
    if (!req.body.password) {
        ...
        updateUserData(userId, first, last, email).then(results =>
        {
            updateProfileData(age, city, url, userId).then(results =>
            {
        ...
        hash(password).then(hash =>
        {
            updateUserDataPwd(userId, first, last, email, hash).then(results =>
            {
                updateProfileData(age, city, url, userId).then(results =>
                {
                    console.log("updateProfileData run")
                ...
                //console.log("results II in edit profile", results)
...
{
    deleteSig(req.session.sigId).then(results =>
    {
